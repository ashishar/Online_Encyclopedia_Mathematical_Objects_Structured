...
export default async function AdminPage() {
  const submitted = await getObjects({ status: "submitted" });
  const reviewed = await getObjects({ status: "reviewed", limit: 12 });
    ...
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <section className="mt-8 rounded-lg border border-line bg-white shadow-panel">
        <div className="border-b border-line px-5 py-4">
          <h2 className="text-lg font-black text-ink">Submitted objects</h2>
        </div>
        <div className="divide-y divide-line">
          {submitted.length ? submitted.map((object) => (
            <div key={object.object_code} className="flex flex-wrap items-center justify-between gap-4 px-5 py-4">
              ...
              <span className="rounded-md bg-paper px-3 py-2 text-sm font-black text-muted">{object.verification_status}</span>
            </div>
          )) : (
            <p className="px-5 py-6 text-muted">No submitted objects found.</p>
          )}
        </div>
      </section>
      ...
      <section className="mt-8 rounded-lg border border-line bg-white shadow-panel">
        <div className="border-b border-line px-5 py-4">
          <h2 className="text-lg font-black text-ink">Needs verification</h2>
        </div>
        <div className="divide-y divide-line">
          {reviewed.map((object) => (
            <div key={object.object_code} className="flex flex-wrap items-center justify-between gap-4 px-5 py-4">
